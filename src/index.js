...
function getProjectFromItem(item) {
  const project = projectArray.find((project) =>
    project.getItems().includes(item)
  );
  return project;
}
...
function masterHandler(e) {
  ...
  if (e.target.id === "item-detail-delete") {
    const item = getItemFromId(
      parseInt(e.target.parentNode.parentNode.dataset.itemId)
    );
  ...
  if (e.target.parentNode.classList.contains("switch-toggle")) {
    if (
      document.querySelector(".active-sidebar-title").id === "side-main-title"
    ) {
      showAllProjects();
    } else {
      const project = getProjectFromId(
        parseInt(document.querySelector(".active-sidebar-title").id)
      );
      showOnlyProject(project);
    }
  }
